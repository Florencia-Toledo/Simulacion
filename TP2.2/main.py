...
#Distribución Normal
datos_normal = [normal(ex_normal, stx_normal) for _ in range(size)] #Genera 1000 valores usando la función de la distribución Normal
...
def pascal_rechazo(k, p):
    c = int(p / (1 - p)) ** k  #Calcula la constante de rechazo 
    while True:
...
def empirica_discreta_rechazo(muestra_empirica):
    ...
    while True:
        x = random.choices(muestra_empirica)[0]  #Selecciona un valor al alzar de la muestra
        fx = p[muestra_empirica.index(x)]  #Probabilidad de x en la muestra
        r = random.uniform(0, c)  #Genera una probabilidad de aceptación
        #Si se cumple la condición, se devuelve el valor, si no se vuelve a generar otro número
        if r <= fx:
            return x 
...
#Distribución Exponencial
muestras_exponencial = [exponencial_rechazo(lam_exponencial) for _ in range(1000)]
...
#Distribución Binomial
muestras_binomial = [binomial_rechazo(n_binomial, p_binomial) for _ in range(1000)]
valores_unicos = np.unique(muestras_binomial)
frecuencias = [muestras_binomial.count(valor) for valor in valores_unicos]
plt.bar(valores_unicos, frecuencias, width=0.01)
plt.xlabel('Valores')
plt.ylabel('Frecuencia')
plt.title('Distribución Binomial')
plt.show()
...
#Distribución Pascal
muestras_pascal = [pascal_rechazo(k_pascal, q_pascal) for _ in range(1000)]
...
#Distribución Hipergeométrica
muestras_hipergeometrica = [hipergeometrica_rechazo(tn_hipergeometrica, ns_hipergeomtrica, p_hipergeometrica) for _ in range(1000)]
